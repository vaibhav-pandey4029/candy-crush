...
function Box({ index,onDrop,onDrag }: IBoxProps) {
  ...
  return (
    <div className={styles.box} draggable onDragStart={()=>onDrag(index)} onDragOver={(e) => e.preventDefault()}  onDrop={()=>onDrop(index)}>
      {index}
    </div>
  )
}
